<!-- resources/views/admin/availabilities/edit.blade.php -->
@extends('layouts.admin')

@section('content')
<div class="admin-page-title">Edit Availability</div>
...
<div class="admin-card">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="admin-card-title">Edit Availability</h2>
        <a href="{{ route('admin.availabilities.index') }}" class="btn-admin-secondary">
            <i class="fas fa-arrow-left"></i> Back to Availabilities
        </a>
    </div>

    <form action="{{ route('admin.availabilities.update', $availability->id) }}" method="POST" class="admin-form">
        @csrf
        @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="coach_id" name="coach_id" {{ !auth()->user()->is_admin ? 'disabled' : '' }} required>
                        ...
                        @foreach($coaches as $coach)
                            <option value="{{ $coach->id }}" {{ old('coach_id', $availability->coach_id) == $coach->id ? 'selected' : '' }}>
                    ...
                    @if(!auth()->user()->is_admin)
                        ...
                        <input type="hidden" name="coach_id" value="{{ $availability->coach_id }}">
                    @endif
                </div>
